...
describe('GoogleDriveService Write Operations', () => {
  ...
  describe('renameFolder', () => {
    ...
    it('should handle special characters in folder names', async () => {
      ...
      const specialNames = ['folder with spaces', 'folder-with-dashes', 'folder (with) parentheses']

      // Create all folders first
      const folders = await Promise.all(
        specialNames.map(async (_, index) => service.createFolder(`temp-folder-${index}`, testRootFolderId)),
      )
      ...
      await Promise.all(folders.map((folder, index) => service.renameFolder(folder.id, specialNames[index])))
  ...
  describe('bulk operations scenario', () => {
    it('should handle a realistic bulk move scenario', async () => {
      ...
      await Promise.all([service.moveFolder(project1.id, projectsFolder.id), service.moveFolder(project2.id, projectsFolder.id)])

      // Rename both in parallel
      await Promise.all([service.renameFolder(project1.id, '2024-Project-Alpha'), service.renameFolder(project2.id, '2024-Project-Beta')])
      ...
      expect(projectNames).toEqual(['2024-Project-Alpha', '2024-Project-Beta'])
  ...
  describe('error handling', () => {
    it('should fail when moving to non-existent parent', async () => {
      ...
      await expect(service.moveFolder(folder.id, 'non-existent-parent-id')).rejects.toThrow()
    })

    it('should fail when renaming non-existent folder', async () => {
      await expect(service.renameFolder('non-existent-folder-id', 'new-name')).rejects.toThrow()
    })

    it('should fail when creating folder with invalid parent', async () => {
      await expect(service.createFolder('orphan', 'invalid-parent-id')).rejects.toThrow()
    })
  })
})
