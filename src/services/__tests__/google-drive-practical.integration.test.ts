...
describe('GoogleDriveService - Practical Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    if (!folderA || !folderB || !testOps) {
      ...
      console.error(
        '   Found:',
        rootContents.files.map((f) => f.name),
      )
  ...
  describe('Read Operations', () => {
    ...
    it('should search for files', async () => {
      ...
      const matchingFiles = result.files.filter((f) => f.name.toLowerCase().includes('test'))
  ...
  describe('Rename Operations', () => {
    it('should rename a file (toggle style)', async () => {
      ...
      const testOpsFiles = await service.listDirectory({ folderId: testOperationsFolderId })
      let renameFile = testOpsFiles.files.find((f) => (f.name === 'file_a.txt' || f.name === 'file_b.txt') && !f.isFolder)
    ...
    it('should rename a folder (toggle style)', async () => {
      ...
      const testOpsFiles = await service.listDirectory({ folderId: testOperationsFolderId })
      let renameFolder = testOpsFiles.files.find((f) => (f.name === 'folder_a' || f.name === 'folder_b') && f.isFolder)
  ...
  describe('Error Handling', () => {
    it('should handle non-existent file gracefully', async () => {
      await expect(service.readFile({ fileId: 'non-existent-file-id-12345' })).rejects.toThrow()
    })

    it('should handle invalid folder ID', async () => {
      await expect(service.listDirectory({ folderId: 'invalid-folder-id-12345' })).rejects.toThrow()
    })

    it('should handle unauthorized access', async () => {
      const badService = new GoogleDriveService('invalid-token', 'invalid-refresh-token', 'invalid-client-id', 'invalid-client-secret')

      await expect(badService.listDirectory({ folderId: 'root' })).rejects.toThrow(
