...
describe('moveFiles tool - folder operations', () => {
  ...
  it('should handle folder names with spaces', async () => {
    ...
    vi.mocked(mockDriveService.resolvePathToId).mockRejectedValueOnce(new Error(`Path not found: ${folderPath}`))
  ...
  it('should handle special characters in folder names', async () => {
    ...
    for (const testCase of testCases) {
      ...
      vi.mocked(mockDriveService.resolvePathToId).mockRejectedValueOnce(new Error(`Path not found: ${testCase.from}`))
