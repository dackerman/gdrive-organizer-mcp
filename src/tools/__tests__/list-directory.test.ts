...
describe('listDirectory tool', () => {
  ...
  it('should filter by onlyDirectories', async () => {
    ...
    expect(content.files.map((f: any) => f.name).sort()).toEqual(['Documents', 'Photos', 'Projects'])
  })
  ...
  it('should handle includeShared parameter', async () => {
    ...
    const listCall = stub.apiCalls.find(
      (call) => call.method === 'filesList' && call.params.q?.includes('in parents') && !call.params.q?.includes('name ='),
    )
